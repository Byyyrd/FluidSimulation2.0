...
public class DensityCalculator : MonoBehaviour
{
    ...
    [SerializeField] public float smoothingRadius;
    ...
    private Vector2 CalculateViscosity(Particle particle)
    {
        ...
        foreach (Particle otherParticle in particle.neighbours)
        {
    ...
    private float CalculateDensity(Vector2 position, List<Particle> neigbours)
    {
        float density = 0;
        float mass = 1;
        float volume = SmootingFunctionVolume(smoothingRadius);
        foreach (Particle particle in neigbours)
        {
            if (particle != null)
            {
                float dist = (position - particle.predictedPosition).magnitude;
                float influence = 0;
                if (dist > nearDensityThreashhold)
                {
    ...
    private float CalculateFieldQuantity(Vector2 position, List<Particle> neigbours)
    {
    ...
    private Vector2 CalculateGradient(Vector2 position, List<Particle> neigbours)
    {
        ...
        foreach (Particle particle in neigbours)
        {
            if (particle != null)
            {
                ...
                resultGradient += CalculateFieldQuantity(particle.position, particle.neighbours) * dir * gradient * mass / (particleDensity + 0.0001f);
            }
        }
